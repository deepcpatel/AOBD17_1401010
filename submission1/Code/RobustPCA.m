function [L, S] = RobustPCA(X, lambda, mu, tol, max_iter)
    ...
    [M, N] = size(X);
    unobserved = isnan(X);  % Finding missing entries
    X(unobserved) = 0;
    normX = norm(X, 'fro'); % Forbenius Norm of X, ||M||_{F}
    ...
    if nargin < 4
        tol = 1e-6;         % Maximum Error Tolerance or delta
    end
    if nargin < 5
        max_iter = 1000;
    end
    ...
    for iter = (1:max_iter) % Augmented Langrage Multiplier Algorithm
        
        % ADMM step: update L and S
        L = Do(1/mu, X - S + (1/mu)*Y);         % Threshold operator
        S = So(lambda/mu, X - L + (1/mu)*Y);    % Shrinkage operator
        ...
        if (err < tol)  % Checking for is error < delta
            break;
        end
    end
end
